...
export default function CharacterDetailPage() {
  ...
  useEffect(() => {
    if (
      !serverId ||
      !decodedCharacterId ||
      !characterInfo?.daevanionBoards?.length
    ) {
      return;
    }
    ...
    <section className="detail-view">
      ...
      {detailTab === "daevanion" && (
        <div className="panel detail-daevanion">
          ...
          {daevanionError && (
            <div className="result-empty">{daevanionError}</div>
          )}
            ...
            <div className="daevanion-body">
              <div className="daevanion-tabs">
                {(characterInfo?.daevanionBoards ?? []).map((board) => {
                  ...
                  const label =
                    daevanionMeta.nameById.get(board.id) ?? `보드 ${board.id}`;
                  ...
                  return (
                    <button
                      ...
                      onClick={() =>
                        setSelectedDaevanionBoardId(board.id ?? null)
                      }
                    >
                      <span>{label}</span>
                      {openText && (
                        <span className="daevanion-tab-sub">({openText})</span>
                      )}
                    </button>
                  );
                })}
              ...
              {(() => {
                ...
                const maxRow = nodes.reduce(
                  (max, node) => Math.max(max, node.row ?? 0),
                  0,
                );
                const maxCol = nodes.reduce(
                  (max, node) => Math.max(max, node.col ?? 0),
                  0,
                );
                const cellSize = 44;
                return (
                        ...
                        {filteredItems.map((item) => {
                          const key =
                            item.id != null
                              ...
                              : `name:${item.name ?? ""}`;
                          const arcanaBonus =
                            skillBonusBySource.arcana.get(key) ?? 0;
                          const gearBonus =
                            skillBonusBySource.gear.get(key) ?? 0;
                          const totalLevel = item.skillLevel ?? 0;
                          const otherBonus = Math.max(
                            0,
                            totalLevel - arcanaBonus - gearBonus,
                          );
                    ...
                    >
                      ...
                      <div className="skill-tooltip-desc">
                        ...
                        (skillTooltip.arcanaBonus === 0 ||
                          ...
                          <>
                            Lv.{skillTooltip.totalLevel} (기타 포인트{" "}
                            {skillTooltip.otherBonus})
                          </>
                        ) : (
                          <>
          ...
          <div className="panel detail-equipment">
            ...
            {Object.entries(equipmentGroups).map(([key, items]) => {
              ...
              return (
                <div key={key}>
                  <div className="subpanel-title">
                    {EQUIPMENT_GROUPS[key as keyof typeof EQUIPMENT_GROUPS]} (
                    {items.length})
                  </div>
                        ...
                        const soulBindSections = (() => {
                          ...
                          const normalizedStats =
                            hasExtend && !alreadyIncluded
                              ? [
                                  { name: "늘어남", value: "적용", icon: null },
                                  ...stats,
                                ]
                              : stats;
                          const skills = (detail.subSkills ?? []).map(
                            (skill) => ({
                              name: skill.name ?? "-",
                              value: `+${skill.level ?? "-"}`,
                      ...
                      >
                        ...
                        {hasArcanaDetail && (
                          <div className="arcana-tooltip">
                            {arcanaMainStats.length > 0 && (
                              <>
                                <div className="arcana-tooltip-title">
                                  기본 옵션
                                </div>
              ...
              {(characterInfo?.statsSummary.primaryStats ?? []).map((stat) => (
                <div className="stat-card" key={stat.type}>
                  <div className="stat-title">
                    <span
                      className={
                        isSkyStatName(stat.name) ? "stat-name-sky" : undefined
                      }
                    >
                      {stat.name}
                    </span>{" "}
